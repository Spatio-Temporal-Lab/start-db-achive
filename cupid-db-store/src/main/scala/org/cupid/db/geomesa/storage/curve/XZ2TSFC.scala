...
import org.cupid.db.geomesa.storage.curve.XZ2TSFC.{QueryWindow, XElement}
...
/**
  ...
  */
class XZ2TSFC(g: Short, xBounds: (Double, Double), yBounds: (Double, Double)) {
  ...
  /**
    ...
    */
  def index(bounds: (Double, Double, Double, Double)): Long =
    index(bounds._1, bounds._2, bounds._3, bounds._4)
  ...
  private def ranges(query: Array[QueryWindow], rangeStop: Int): Seq[IndexRange] = {
    ...
    import XZ2TSFC.{LevelOneElements, LevelTerminator}
...
object XZ2TSFC {
  ...
  private val cache = new java.util.concurrent.ConcurrentHashMap[Short, XZ2TSFC]()

  def apply(g: Short): XZ2TSFC = {
    var sfc = cache.get(g)
    if (sfc == null) {
      sfc = new XZ2TSFC(g, (-180.0, 180.0), (-90.0, 90.0))
      cache.put(g, sfc)
    }
    sfc
  }
