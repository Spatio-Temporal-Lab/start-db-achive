...
public class SqlValidatorImpl implements SqlValidatorWithHints {
    ...
    private final Map<SqlSelect, SqlBasicCall> selectTableFunctions = new IdentityHashMap<>();
    ...
    private static SqlNode expandExprFromJoin(
        ...
        for (SqlNode node : (SqlNodeList) join.getCondition()) {
            ...
            if (identifier.getSimple().equals(name)) {
                ...
                final SqlNode finalNode = SqlStdOperatorTable.AS.createCall(
                    SqlParserPos.ZERO,
    ...
    public CalciteException handleUnresolvedFunction(
        ...
        AssignableOperandTypeChecker typeChecking = new AssignableOperandTypeChecker(
            argTypes,
            argNames
        );
        String signature = typeChecking.getAllowedSignatures(
            unresolvedFunction,
    ...
    protected void inferUnknownTypes(
        ...
        if ((node instanceof SqlDynamicParam) || isNullLiteral) {
            ...
            final RelDataType whenType = caseCall.getValueOperand() == null
                ? booleanType
                : unknownType;
            ...
            final SqlCall call = (SqlCall) node;
            final SqlOperandTypeInference operandTypeInference = call.getOperator()
    ...
    private void registerMatchRecognize(
        ...
        final MatchRecognizeNamespace matchRecognizeNamespace = createMatchRecognizeNameSpace(
            call,
            enclosingNode
        );
    ...
    private void registerQuery(
        ...
        switch (node.getKind()) {
            case SELECT:
                ...
                final SqlValidatorScope windowParentScope = (usingScope != null)
                    ? usingScope
                    : parentScope;
                ...
                if (select.getGroup() != null) {
                    GroupByScope groupByScope = new GroupByScope(
                        selectScope,
                        select.getGroup(),
                        select
                    );
            ...
            case UNNEST:
                call = (SqlCall) node;
                final UnnestNamespace unnestNs = new UnnestNamespace(
                    this,
                    call,
                    parentScope,
                    enclosingNode
            ...
            case MULTISET_VALUE_CONSTRUCTOR:
                ...
                final CollectNamespace tableConstructorNs = new CollectNamespace(
                    call,
                    cs,
                    enclosingNode
                );
    ...
    protected boolean isNestedAggregateWindow(SqlNode node) {
        AggFinder nestedAggFinder = new AggFinder(
            opTab,
            false,
            false,
            false,
        ...
        switch (literal.getTypeName()) {
            ...
            case INTERVAL_SECOND:
                if (literal instanceof SqlIntervalLiteral) {
                    SqlIntervalLiteral.IntervalValue interval = literal.getValueAs(
    ...
    protected void validateJoin(SqlJoin join, SqlValidatorScope scope) {
        ...
        if (natural) {
            ...
            final SqlNameMatcher nameMatcher = catalogReader.nameMatcher();
            List<String> naturalColumnNames = SqlValidatorUtil.deriveNaturalJoinColumnList(
                nameMatcher,
                leftRowType,
                rightRowType
            );
        ...
        if (current instanceof SqlCall && !(current instanceof SqlSelect)) {
            ...
            if (!id.isStar() && isRolledUpColumn(id, scope)) {
                if (!isAggregation(parent.getKind())
                    ...
                    String context = optionalClause != null
                        ? optionalClause
                        : parent.getKind().toString();
                    throw newValidationError(
                        id,
    ...
    protected void validateWindowClause(SqlSelect select) {
        ...
        for (SqlWindow window : windows) {
            final SqlNodeList expandedOrderList = (SqlNodeList) expand(
                window.getOrderList(),
                windowScope
            );
            ...
            final SqlNodeList expandedPartitionList = (SqlNodeList) expand(
    ...
    protected RelDataType createTargetRowType(
        ...
        if (append) {
            for (RelDataTypeField targetField : targetFields) {
                fields.add(
                    Pair.of(SqlUtil.deriveAliasFromOrdinal(fields.size()), targetField.getType())
                );
            }
        }
    ...
    public void validateInsert(SqlInsert insert) {
        ...
        final List<ColumnStrategy> strategies = table.unwrap(RelOptTable.class)
            .getColumnStrategies();
        ...
        final RelDataType targetRowTypeToValidate = logicalSourceRowType
            .getFieldCount() == logicalTargetRowType.getFieldCount()
    ...
    private void checkConstraint(
        ...
        final ModifiableViewTable modifiableViewTable = validatorTable.unwrap(
            ModifiableViewTable.class
        );
        if (modifiableViewTable != null && source instanceof SqlCall) {
            ...
            final Map<Integer, RelDataTypeField> tableIndexToTargetField = SqlValidatorUtil
                .getIndexToFieldMap(tableFields, targetRowType);
            ...
            final ImmutableBitSet targetColumns = ImmutableBitSet.of(
                tableIndexToTargetField.keySet()
            );
            final ImmutableBitSet constrainedColumns = ImmutableBitSet.of(projectMap.keySet());
            final ImmutableBitSet constrainedTargetColumns = targetColumns.intersect(
                constrainedColumns
            );
    ...
    private void checkConstraint(
        ...
        if (modifiableViewTable != null) {
            ...
            final Map<String, Integer> nameToIndex = SqlValidatorUtil.mapNameToIndex(
    ...
    private void checkFieldCount(
        ...
        for (final RelDataTypeField field : table.getRowType().getFieldList()) {
            final RelDataTypeField targetField = targetRowTypeToValidate.getField(
                field.getName(),
                true,
                false
            );
    ...
    protected void checkTypeAssignment(
        ...
        if (config.typeCoercionEnabled() && !isUpdateModifiableViewTable) {
            ...
            boolean coerced = typeCoercion.querySourceCoercion(
                sourceScope,
                sourceRowType,
                targetRowType,
                query
    ...
    public void validateMerge(SqlMerge call) {
        ...
        IdentifierNamespace targetNamespace = (IdentifierNamespace) getNamespace(
            call.getTargetTable()
        );
        validateNamespace(targetNamespace, unknownType);
    ...
    protected void validateValues(
        ...
        for (SqlNode operand : operands) {
            ...
            if (targetRowType.isStruct()) {
                for (Pair<SqlNode, RelDataTypeField> pair : Pair.zip(
    ...
    class ValidationErrorFunction
        implements
            Function2<SqlNode, Resources.ExInst<SqlValidatorException>, CalciteContextException> {
        @Override
    ...
    public void validateMatchRecognize(SqlCall call) {
        ...
        final MatchRecognizeScope scope = (MatchRecognizeScope) getMatchRecognizeScope(
            matchRecognize
        );
        ...
        List<Map.Entry<String, RelDataType>> measureColumns = validateMeasure(
            matchRecognize,
            scope,
            allRows
        );
    ...
    private SqlNode navigationInMeasure(SqlNode node, boolean allRows) {
        ...
        final SqlOperator defaultOp = allRows
    ...
    public void validatePivot(SqlPivot pivot) {
        ...
        pivot.forEachNameValues((alias, nodeList) -> {
            if (nodeList.size() != axisTypes.size()) {
                throw newValidationError(
                    nodeList,
                    RESOURCE.pivotValueArityMismatch(nodeList.size(), axisTypes.size())
                );
            }
            ...
            Pair.forEach(
                aggNames,
                (aggAlias, aggType) -> typeBuilder.add(
    ...
    static class SelectExpander extends Expander {
        ...
        @Override
        public SqlNode visit(SqlIdentifier id) {
            final SqlNode node = expandCommonColumn(
                select,
                id,
                (SelectScope) getScope(),
                validator
            );
            if (node != id) {
                return node;
            ...
            if (isLogicalNavigation(kind) || isPhysicalNavigation(kind)) {
                ...
                if (isPhysicalNavigation(kind)) {
                    ...
                    if (isPhysicalNavigation(innerKind)) {
                        ...
                        inner = call.getOperator()
                            .createCall(SqlParserPos.ZERO, innerOperands.get(0), offset);
                    }
                }
                SqlNode newInnerNode = inner.accept(
                    new NavigationExpander(call.getOperator(), offset)
                );
                if (op != null) {
                ...
                switch (kind) {
                    ...
                    default:
                        if (operands.size() == 0
                            ...
                            || ((SqlCall) operands.get(0))
                                .getOperator() != SqlStdOperatorTable.CLASSIFIER) {
                            if (vars.isEmpty()) {
                                throw newValidationError(
                                    call,
        ...
        Permute(SqlNode from, int offset) {
            switch (from.getKind()) {
                case JOIN:
                    ...
                    final int fieldCount = getValidatedNodeType(join.getLeft()).getFieldList()
                        .size();
                ...
                default:
                    ...
                    this.sources = Functions.generate(
        ...
        public void permute(
            ...
            for (ImmutableIntList source : sources) {
                ...
                for (int p1 : Util.skip(source)) {
                    ...
                    final RelDataType type2 = SqlTypeUtil.leastRestrictiveForComparison(
                        typeFactory,
                        type,
                        type1
                    );
