...
public class EntityAccessorTest {
    ...
    public void userTest() {
        try(UserAccessor userAccessor = AccessorFactory.getUserAccessor()) {
            ...
            int size = users.size();
            Assert.assertEquals("wrong", "User{id=1, name='Wangbohong', password='beecorn'}", user1.toString());
    ...
    public void test1() throws Exception{
        try(TableAccessor tableAccessor = AccessorFactory.getTableAccessor()) {
            ...
            Assert.assertEquals("wrong", "Table{id=1, name='test', dbId=1, storageEngine='default'}", table1.toString());
            Assert.assertEquals(1, tableAccessor.insert(new Table(1L,1L, name, "default")));
            Assert.assertEquals(1L, tableAccessor.update(new Table(2L, 1L, name + "_tmp", "default")));
