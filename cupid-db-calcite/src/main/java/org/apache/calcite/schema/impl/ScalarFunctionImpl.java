...
import org.urbcomp.cupid.db.function.CupidDBFunction;
...
public class ScalarFunctionImpl extends ReflectiveFunctionBase
    ...
    public static ImmutableMultimap<String, Function> functions(Class<?> clazz) {
        ...
        for (Method method : clazz.getMethods()) {
            ...
            boolean isAnnotationPresent = method.isAnnotationPresent(CupidDBFunction.class);
    ...
    private static String getFunctionName(Method method, boolean isAnnotationPresent) {
        if (isAnnotationPresent) {
            return method.getAnnotation(CupidDBFunction.class).value();
        } else {
