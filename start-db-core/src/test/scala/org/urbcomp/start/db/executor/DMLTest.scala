...
class DMLTest extends AbstractCalciteCRUDTest{
  ...
  test("basic insert and delete") {
    ...
    val i1 = statement.executeUpdate("insert into t_crud_test (idx, rs, traj) values (1, st_rs_fromGeoJSON(\'" + rsGeoJson + "\'), st_traj_fromGeoJSON(\'" + tGeo + "\'))")
    assertEquals(1, i1)
    // insert into t (a, b) values (1, 2)
    val i2 = statement.executeUpdate("insert into t_crud_test values (2, st_rs_fromGeoJSON(\'" + rsGeoJson + "\'), st_traj_fromGeoJSON(\'" + tGeo + "\'))")
    assertEquals(1, i2)
  ...
  test("multiple insert") {
    ...
    val i2 = statement.executeUpdate("insert into t_crud_test (idx, rs) values (3, st_rs_fromGeoJSON(\'" + rsGeoJson + "\')), (4, st_rs_fromGeoJSON(\'" + rsGeoJson + "\'))")
    assertEquals(2, i2)
   ...
   */
  test ("update test") {
    statement.executeUpdate("insert into t_crud_test (idx, rs, traj) values (1, st_rs_fromGeoJSON(\'" + rsGeoJson + "\'), st_traj_fromGeoJSON(\'" + tGeo + "\'))")
