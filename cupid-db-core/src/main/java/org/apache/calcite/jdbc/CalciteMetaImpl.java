...
public class CalciteMetaImpl extends MetaImpl {
    ...
    Enumerable<MetaTableType> tableTypes() {
        return Linq4j.asEnumerable(
            ImmutableList.of(new MetaTableType("TABLE"), new MetaTableType("VIEW"))
        );
    }
    ...
    Enumerable<MetaSchema> schemas(final String catalog) {
        return Linq4j.asEnumerable(getConnection().rootSchema.getSubSchemaMap().values())
            .select(
                (Function1<CalciteSchema, MetaSchema>) calciteSchema -> new CalciteMetaSchema(
                    calciteSchema,
                    catalog,
            ...
            .orderBy(
                (Function1<MetaSchema, Comparable>) metaSchema -> (Comparable) FlatLists.of(
    ...
    Enumerable<MetaTable> tables(String catalog) {
        return schemas(catalog).selectMany(
            schema -> tables(schema, Functions.<String>truePredicate1())
        );
    }
    ...
    Enumerable<MetaTable> tables(final MetaSchema schema_) {
        ...
        return Linq4j.asEnumerable(schema.calciteSchema.getTableNames())
            ...
            .concat(
                Linq4j.asEnumerable(
                    schema.calciteSchema.getTablesBasedOnNullaryFunctions().entrySet()
                ).select(pair -> {
                    final Table table = pair.getValue();
                    return new CalciteMetaTable(
                        table,
                        schema.tableCatalog,
                        schema.tableSchem,
                        pair.getKey()
    ...
    public Enumerable<MetaColumn> columns(final MetaTable table_) {
        ...
        return Linq4j.asEnumerable(rowType.getFieldList()).select(field -> {
    ...
    Iterable<Object> _createIterable(
        StatementHandle handle,
        Signature signature,
        List<TypedValue> parameterValues,
        Frame firstFrame
            ...
            synchronized (callback.getMonitor()) {
                ...
                switch (signature.statementType) {
                    ...
                    case OTHER_DDL:
                        ...
                        // cupid-db modify start we need the updateCount
                        updateCount = (int) signature.getMaxRowCount();
                        // cupid-db modify end
                        break;
                    default:
    ...
    private CalcitePrepare.Query<Object> toQuery(Context context, String sql) {
        final Holder<CalcitePrepare.Query<Object>> queryHolder = Holder.of(
            CalcitePrepare.Query.of(sql)
        );
    ...
    public Frame fetch(StatementHandle h, long offset, int fetchMaxRowCount)
        ...
        final List rows = MetaImpl.collect(
    ...
    public ExecuteResult execute(
        ...
        if (signature.statementType.canUpdate()) {
            ...
            stmt.setResultSet(iterator);
            metaResultSet = MetaResultSet.count(
                h.connectionId,
                h.id,
    ...
    public ExecuteBatchResult prepareAndExecuteBatch(
        ...
        final Meta.PrepareCallback callback = new Meta.PrepareCallback() {
            ...
            public void execute() throws SQLException {
                if (signature.statementType.canUpdate()) {
                    final Iterable<Object> iterable = _createIterable(
                        h,
                        signature,
                        ImmutableList.of(),
                        null
    ...
    public static DataContext createDataContext(CalciteConnection connection) {
        return ((CalciteConnectionImpl) connection).createDataContext(
            ImmutableMap.of(),
    ...
    private static class CalciteMetaTable extends MetaTable {
        ...
        CalciteMetaTable(Table calciteTable, String tableCat, String tableSchem, String tableName) {
            super(tableCat, tableSchem, tableName, calciteTable.getJdbcTableType().jdbcName);
