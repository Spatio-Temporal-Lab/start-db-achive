  ...
  "Z2TIndex" should {
    "index and query yearly epochs correctly" in {
      foreach(Seq("z2t:1:dtg")) { indices =>
        ...
        val features =
          (0 until 10).map { i =>
            ScalaSimpleFeature.create(sft, s"$i", s"name$i", "track1", s"2022-12-07T0$i:00:00.000Z", s"POINT(4$i 60)")
          } ++ (10 until 20).map { i =>
            ScalaSimpleFeature.create(sft, s"$i", s"name$i", "track2", s"2022-12-${i}T$i:00:00.000Z", s"POINT(4${i - 10} 60)")
          } ++ (20 until 30).map { i =>
            ScalaSimpleFeature.create(sft, s"$i", s"name$i", "track3", s"2022-12-${i}T${i-10}:00:00.000Z", s"POINT(6${i - 20} 60)")
          } ++ (30 until 32).map { i =>
            ScalaSimpleFeature.create(sft, s"$i", s"name$i", "track4", s"2022-12-${i}T${i-10}:00:00.000Z", s"POINT(${i - 20} 60)")
          }
