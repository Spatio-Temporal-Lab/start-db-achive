    ...
    public RelDataType inferReturnType(SqlOperatorBinding opBinding) {
        ...
        if (message != null) {
            throw callBinding.newValidationError(
                RESOURCE.wrongNumberOfParam(getName(), thisOperands.length, message)
            );
        }

        final SqlCall newCall = getLookupCall();
        final SqlCallBinding newBinding = new SqlCallBinding(
        ...
        private JdbcToInternalLookupTable() {
            ...
            map.put("CONVERT", new SimpleMakeCall(SqlStdOperatorTable.CAST) {
                ...
                public SqlCall createCall(SqlParserPos pos, SqlNode... operands) {
                    ...
                    SqlJdbcDataTypeName jdbcType = ((SqlLiteral) typeOperand).symbolValue(
                        SqlJdbcDataTypeName.class
                    );

                    return super.createCall(
                        pos,
                        operands[0],
                        jdbcType.createDataType(typeOperand.pos)
                    );
                }
            });
            this.map = map.build();
