 ...
 */
abstract class CalciteConnectionImpl extends AvaticaConnection
    implements
        CalciteConnection,
        QueryProvider {
     ...
     */
    protected CalciteConnectionImpl(
        Driver driver,
        ...
        if (typeFactory != null) {
            ...
            RelDataTypeSystem typeSystem = cfg.typeSystem(
        ...
        this.rootSchema = Objects.requireNonNull(
            rootSchema != null
                ...
                : CalciteSchema.createRootSchema(
                    true,
                    // start-db modify disable cache
                    Boolean.parseBoolean(info.getProperty("useFullSchemaCache", "false"))
                )
        );
        // start-db modify end
    ...
    @Override
    public CalciteStatement createStatement(
        int resultSetType,
        int resultSetConcurrency,
        int resultSetHoldability
    ) throws SQLException {
        return (CalciteStatement) super.createStatement(
            resultSetType,
    ...
    public <T> Enumerable<T> enumerable(
    ...
    private static class CalciteServerImpl implements CalciteServer {
        ...
        public void addStatement(CalciteConnection connection, Meta.StatementHandle h) {
            ...
            final CalciteServerStatement previous = statementMap.put(
                h.id,
                new CalciteServerStatementImpl(c)
            );
            if (previous != null) {
    ...
    static class DataContextImpl implements DataContext {
        ...
        DataContextImpl(
            ...
            final Locale locale = localeName != null ? Util.parseLocale(localeName) : Locale.ROOT;

            // Give a hook chance to alter standard input, output, error streams.
            final Holder<Object[]> streamHolder = Holder.of(
                new Object[] { System.in, System.out, System.err }
            );
        ...
        private SqlAdvisor getSqlAdvisor() {
            ...
            final List<String> schemaPath = schemaName == null
                ? ImmutableList.of()
                : ImmutableList.of(schemaName);
            final SqlValidatorWithHints validator = new SqlAdvisorValidator(
                SqlStdOperatorTable.instance(),
                new CalciteCatalogReader(rootSchema, schemaPath, typeFactory, con.config()),
                typeFactory,
