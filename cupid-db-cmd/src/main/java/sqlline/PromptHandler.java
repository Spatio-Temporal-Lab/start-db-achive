...
public class PromptHandler {
    private static final Map<Character, Supplier<String>> DATE_TIME_FORMATS = Collections
        .unmodifiableMap(new HashMap<Character, Supplier<String>>() {
            {
        ...
        return new MemoizingSupplier<>(() -> {
            ...
            if (scriptEngine == null) {
                if (engineManager.getEngineFactories().isEmpty()) {
                    sqlLine.error(
                        sqlLine.loc("not-supported-script-engine-no-available", engineName)
                    );
                } else {
                    sqlLine.error(
                        sqlLine.loc(
                            "not-supported-script-engine",
                            engineName,
                            BuiltInProperty.SCRIPT_ENGINE.getAvailableValues()
                        )
                    );
                }
            }
    ...
    public AttributedString getRightPrompt() {
        ...
        final String currentPrompt = sqlLine.getOpts().isDefault(BuiltInProperty.RIGHT_PROMPT)
    ...
    public AttributedString getPrompt() {
        ...
        if (!sqlLine.getOpts().isDefault(BuiltInProperty.PROMPT_SCRIPT)) {
            final String promptScript = sqlLine.getOpts().get(BuiltInProperty.PROMPT_SCRIPT);
            return getPrompt(sqlLine, connectionIndex, getPromptFromScript(sqlLine, promptScript));
        }
        final String defaultPrompt = String.valueOf(BuiltInProperty.PROMPT.defaultValue());
        ...
        final boolean useDefaultPrompt = sqlLine.getOpts().isDefault(BuiltInProperty.PROMPT);
        final String prompt;
    ...
    private String getPromptFromScript(SqlLine sqlLine, String promptScript) {
        try {
